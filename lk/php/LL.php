...
function debugMatching($tempStorage, $db) {
    ...
    foreach ($leftoversData as $row) {
        ...
        if (count($row) >= 3) {
            ...
            $quantity = isset($row[2]) ? (float)str_replace([' ', ','], ['', '.'], $row[2]) : 0;
    ...
    $query = "SELECT DISTINCT beer_name FROM get_basebeer WHERE beer_name IS NOT NULL";
...
{
    $query =
        "SELECT p.id as p_id, p.id_tare, p.id_shop, p.order, p.id_beer, c.cost, g.beer_name, g.beer_dist, g.beer_id, g.brewery_name, c.id as cost_id, p.count as count_base " .
        "FROM purchase p " .
...
class TempFileStorage {
    ...
    public function __construct() {
        ...
        $this->filename = $this->tempDir . 'leftovers_' . session_id() . '.json';
    }
    ...
    public function save($data) {
        ...
        return file_put_contents($this->filename, json_encode($fileData, JSON_UNESCAPED_UNICODE));
    }
...
function createAkt($db, $id_shop, $tempStorage) {
    ...
    if (!$tempStorage->exists() || !$tempStorage->isRecent()) {
        echo json_encode([
            "success" => false,
            "message" => "Нет актуальных данных из файла остатков. Сначала загрузите файл."
        ]);
        return;
    }
    ...
    $query = "SELECT p.id as p_id, p.id_tare, p.id_shop, p.order, p.id_beer, c.cost,
                     ...
                     c.id as cost_id, p.count as count_base
              FROM purchase p
              LEFT JOIN tare t ON p.id_tare = t.id
              LEFT JOIN get_basebeer g ON p.id_beer = g.beer_id
              LEFT JOIN purchase_cost c ON p.ID = c.id_parent
    ...
    $table1Data = [
        ...
        [
            '<b>Пивоварня</b>',
            '<b>Сорт</b>',
            '<b><wraptext>Количество в базе</wraptext></b>',
            '<b><wraptext>Количество в 1С</wraptext></b>',
            '<b><wraptext>Расхождение</wraptext></b>',
            '<b><wraptext>Фактическое наличие</wraptext></b>'
        ]
    ];
    ...
    foreach ($purchaseData as $normalizedName => $purchaseItem) {
        ...
        $differenceText = $difference == 0 ? '' : ($difference > 0 ? '+' . $difference : $difference);

        $table1Data[] = [
                "<left>$breweryName</left>",
                "<left>$beerName</left>",
                "<center>$countBase</center>",
                "<center>$count1C</center>",
                "<center>$differenceText</center>",
            ''
        ];
    ...
    $table1Data[] = [
        ...
        '<i>'.$table1TotalBase.'</i>',
        '<i>'.$table1Total1C.'</i>',
        '<i>'. ($table1TotalBase - $table1Total1C) .'</i>',
        ''
    ];
    ...
    $aktData = [
        ['<b>Акт инвентаризации от ' . $currentDate . ' для BeetleCraft / Пенза, Московская 2</b>'],
    ...
    $aktData[] = ['Товаров в базе (get_basebeer) кроме purchase: ' . count($allBeerFromDB)];
    ...
    // 9. Создаем Excel
    $xlsx = \Shuchkin\SimpleXLSXGen::fromArray($aktData)
        ->mergeCells('A1:F1')
        ->mergeCells('A5:F5')
        ->mergeCells('A25:C25')
        ->setColWidth(1, 20)
